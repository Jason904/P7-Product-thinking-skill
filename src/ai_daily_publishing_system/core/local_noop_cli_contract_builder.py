"""Build pure local noop CLI/manual entry contract explanations."""

from typing import Final


REASON_CODES: Final[tuple[str, ...]] = (
    "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE",
    "RUN_ID_MISSING",
    "LOCAL_NOOP_CLI_CONTRACT_ID_MISSING",
    "CONTRACT_KIND_NOT_LOCAL_NOOP_CLI_CONTRACT",
    "ENTRY_MODE_NOT_SUPPORTED",
    "MODE_NOT_NOOP",
    ...
    "CLI_CONTRACT_EVIDENCE_ITEMS_MISSING",
    "REQUIRED_CLI_CONTRACT_EVIDENCE_IDS_MISSING",
    "MISSING_CLI_CONTRACT_EVIDENCE_IDS_DECLARED",
    "BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN",
    "CREATED_AT_MISSING",
    "TIMESTAMP_POLICY_MISSING",
    "SOURCE_OF_TRUTH_MISSING",
    "CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT",
    "CLI_CONTRACT_EVIDENCE_ITEM_KEYS_INVALID",
    "CLI_CONTRACT_EVIDENCE_ID_MISSING",
    "CLI_CONTRACT_EVIDENCE_ROLE_MISSING",
    "CLI_CONTRACT_EVIDENCE_ARTIFACT_REF_MISSING",
    "CLI_CONTRACT_EVIDENCE_ARTIFACT_KIND_MISSING",
    "CLI_CONTRACT_EVIDENCE_STATUS_MISSING",
    "CLI_CONTRACT_EVIDENCE_PRODUCER_REF_MISSING",
    "CLI_CONTRACT_EVIDENCE_REFS_MISSING",
    "CLI_CONTRACT_EVIDENCE_ID_DUPLICATE",
    "CLI_CONTRACT_EVIDENCE_ID_NOT_REQUIRED",
    "REQUIRED_CLI_CONTRACT_EVIDENCE_MISSING",
    "CLI_CONTRACT_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT",
)

LOCAL_NOOP_CLI_CONTRACT_BUILD_REASON_CODES: Final[tuple[str, ...]] = (
    REASON_CODES
)
...
_CONTRACT_KIND: Final[str] = "local_noop_cli_contract"
_NOOP_MODE: Final[str] = "noop"
_NOOP_COMPLETED: Final[str] = "NOOP_COMPLETED"
_PASS_PUBLISHED: Final[str] = "PASS_PUBLISHED"
_SUPPORTED_ENTRY_MODES: Final[tuple[str, ...]] = ("cli", "manual")

_CLI_CONTRACT_EVIDENCE_ITEM_KEYS: Final[tuple[str, ...]] = (
    "cli_contract_evidence_id",
    "cli_contract_evidence_role",
    "artifact_ref",
    "artifact_kind",
    "evidence_status",
    "producer_ref",
...
_CLI_CONTRACT_EVIDENCE_ITEM_STRING_FIELDS: Final[
    tuple[tuple[str, str], ...]
] = (
    ("cli_contract_evidence_id", "CLI_CONTRACT_EVIDENCE_ID_MISSING"),
    ("cli_contract_evidence_role", "CLI_CONTRACT_EVIDENCE_ROLE_MISSING"),
    ("artifact_ref", "CLI_CONTRACT_EVIDENCE_ARTIFACT_REF_MISSING"),
    ("artifact_kind", "CLI_CONTRACT_EVIDENCE_ARTIFACT_KIND_MISSING"),
    ("evidence_status", "CLI_CONTRACT_EVIDENCE_STATUS_MISSING"),
    ("producer_ref", "CLI_CONTRACT_EVIDENCE_PRODUCER_REF_MISSING"),
)

_FORBIDDEN_CLI_CONTRACT_EVIDENCE_ITEM_FIELDS: Final[tuple[str, ...]] = (
...
_INVARIANT_REFS: Final[tuple[str, ...]] = (
    "local_noop_cli_contract_builder_only",
    ...
    "cli_contract_evidence_items_are_caller_supplied",
    "cli_contract_evidence_status_is_caller_supplied",
    ...
    "local_noop_cli_contract_governance_evidence_bundle",
    "local_noop_cli_contract_not_cli_execution",
    "local_noop_cli_contract_not_manual_execution",
    "local_noop_cli_contract_not_argument_parsing",
    "local_noop_cli_contract_not_console_script",
    "local_noop_cli_contract_not_command_execution",
    "local_noop_cli_contract_not_subprocess_execution",
    "local_noop_cli_contract_not_ledger_write",
    "local_noop_cli_contract_not_run_ledger_yaml",
    "local_noop_cli_contract_not_runner_execution",
    "local_noop_cli_contract_not_runtime_execution",
    "local_noop_cli_contract_not_state_transition",
    "local_noop_cli_contract_not_gate_decision",
    "local_noop_cli_contract_not_publish_artifact",
    "local_noop_cli_contract_not_public_candidate",
    "entry_mode_is_declarative_only",
    "mode_noop_required",
    ...
    "cli_contract_evidence_status_not_quality_pass",
    "cli_contract_evidence_status_not_gate_pass",
    "cli_contract_evidence_status_not_publish_allowed",
    ...
    "blocking_cli_contract_evidence_ids_are_evidence_only",
    "blocking_cli_contract_evidence_ids_do_not_execute_gate",
    "blocking_cli_contract_evidence_ids_do_not_execute_noop_completion",
    "blocking_cli_contract_evidence_ids_do_not_execute_dry_run",
    "blocking_cli_contract_evidence_ids_do_not_execute_e2e",
    "blocking_cli_contract_evidence_ids_do_not_execute_runner",
    "blocking_cli_contract_evidence_ids_do_not_write_ledger",
    "no_reader_read",
...
_REASON_TEXT_ENTRIES: Final[tuple[tuple[str, str], ...]] = (
    ...
    (
        "LOCAL_NOOP_CLI_CONTRACT_ID_MISSING",
        "A non-empty caller-supplied local_noop_cli_contract_id is required.",
    ),
    (
        "CONTRACT_KIND_NOT_LOCAL_NOOP_CLI_CONTRACT",
        "contract_kind must be local_noop_cli_contract.",
    ),
    (
        "ENTRY_MODE_NOT_SUPPORTED",
        "entry_mode must be cli or manual, and remains declarative only.",
    ),
    ("MODE_NOT_NOOP", "mode must be noop."),
    (
        "PASS_PUBLISHED_FORBIDDEN",
        "PASS_PUBLISHED is forbidden for local noop CLI contracts.",
    ),
    (
        "EXPECTED_TERMINAL_STATUS_NOT_NOOP_COMPLETED",
    ...
    (
        "GATE_INPUT_REF_MISSING",
        "A caller-supplied gate input ref is required.",
    ),
    (
        "GATE_INPUT_BUILDABLE_MARKER_NOT_TRUE",
    ...
    (
        "CLI_CONTRACT_EVIDENCE_ITEMS_MISSING",
        "At least one CLI contract evidence item is required.",
    ),
    (
        "REQUIRED_CLI_CONTRACT_EVIDENCE_IDS_MISSING",
        "At least one required CLI contract evidence id is required.",
    ),
    (
        "MISSING_CLI_CONTRACT_EVIDENCE_IDS_DECLARED",
        "missing_cli_contract_evidence_ids must be an empty tuple.",
    ),
    (
        "BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN",
        "Blocking CLI contract evidence ids must be known non-empty ids.",
    ),
    ...
    (
        "CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT",
        "Every CLI contract evidence item must be a dict.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ITEM_KEYS_INVALID",
        "Every CLI contract evidence item must contain the exact expected keys.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ID_MISSING",
        "Every CLI contract evidence item requires a non-empty "
        "cli_contract_evidence_id.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ROLE_MISSING",
    ...
    (
        "CLI_CONTRACT_EVIDENCE_ARTIFACT_REF_MISSING",
        "Every CLI contract evidence item requires a non-empty artifact_ref.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ARTIFACT_KIND_MISSING",
        "Every CLI contract evidence item requires a non-empty artifact_kind.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_STATUS_MISSING",
        "Every CLI contract evidence item requires a non-empty evidence_status.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_PRODUCER_REF_MISSING",
        "Every CLI contract evidence item requires a non-empty producer_ref.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_REFS_MISSING",
        "Every CLI contract evidence item requires non-empty evidence_refs.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ID_DUPLICATE",
        "cli_contract_evidence_id values must be unique.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ID_NOT_REQUIRED",
        "Every cli_contract_evidence_id must be declared as required.",
    ),
    (
        "REQUIRED_CLI_CONTRACT_EVIDENCE_MISSING",
        "Every required CLI contract evidence id must have one evidence item.",
    ),
    (
        "CLI_CONTRACT_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT",
        "CLI contract evidence items must not contain raw, execution, CLI, "
        "command, URL, IO, publish, ledger, notification, or policy fields.",
    ),
    (
        "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE",
        "The caller-supplied fields can build the local noop CLI/manual "
        "entry contract shape. This does not execute CLI, manual commands, "
        "shell, subprocess, runner, runtime, dry-run, E2E, noop completion, "
        "transition, gate, policy, validator, eval, audit, publish, ledger "
        "write, notification, artifact read, existing builder call, or public "
        "URL behavior.",
    ),
)
...
def _safe_cli_contract_evidence_item(
    cli_contract_evidence_item: object,
) -> dict[str, object]:
    if not isinstance(cli_contract_evidence_item, dict):
        return {
            "cli_contract_evidence_id": "",
            "cli_contract_evidence_role": "",
            "artifact_ref": "",
    ...
    return {
        "cli_contract_evidence_id": _safe_string(
            cli_contract_evidence_item.get("cli_contract_evidence_id")
        ),
        "cli_contract_evidence_role": _safe_string(
            cli_contract_evidence_item.get("cli_contract_evidence_role")
        ),
        "artifact_ref": _safe_string(cli_contract_evidence_item.get("artifact_ref")),
        "artifact_kind": _safe_string(cli_contract_evidence_item.get("artifact_kind")),
        "evidence_status": _safe_string(
            cli_contract_evidence_item.get("evidence_status")
        ),
        "producer_ref": _safe_string(cli_contract_evidence_item.get("producer_ref")),
        "evidence_refs": _safe_string_tuple(
            cli_contract_evidence_item.get("evidence_refs")
        ),
        "notes": _safe_string_tuple(cli_contract_evidence_item.get("notes")),
    }


def _safe_cli_contract_evidence_items(
    value: object,
    ...
    safe_items = ()
    for cli_contract_evidence_item in value:
        safe_items = safe_items + (
            _safe_cli_contract_evidence_item(cli_contract_evidence_item),
        )
    return safe_items
...
def _reason_text(reason_code: str) -> str:
    for code, text in _REASON_TEXT_ENTRIES:
        if code == reason_code:
            return text
    return "Unknown local noop CLI contract build result."
...
def _add_cli_contract_evidence_item_violation(
    cli_contract_evidence_item_violations: list[dict[str, object]],
    *,
    cli_contract_evidence_item_index: int,
    cli_contract_evidence_id: str,
    reason_code: str,
    field: str,
) -> None:
    cli_contract_evidence_item_violations.append(
        {
            "cli_contract_evidence_item_index": cli_contract_evidence_item_index,
            "cli_contract_evidence_id": cli_contract_evidence_id,
...
def _ordered_reason_codes(reason_codes: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for reason_code in REASON_PRIORITY:
        if reason_code == "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE":
            continue
...
def _ordered_cli_contract_evidence_item_violations(
    cli_contract_evidence_item_violations: tuple[dict[str, object], ...],
) -> tuple[dict[str, object], ...]:
    return tuple(
        sorted(
            cli_contract_evidence_item_violations,
            key=lambda item: (
                _reason_rank(_safe_string(item.get("reason_code"))),
                item.get("cli_contract_evidence_item_index"),
...
def _cli_contract_evidence_id_from(cli_contract_evidence_item: object) -> str:
    if not isinstance(cli_contract_evidence_item, dict):
        return ""
    return _safe_string(cli_contract_evidence_item.get("cli_contract_evidence_id"))


def _known_cli_contract_evidence_ids(
    cli_contract_evidence_items: object,
) -> tuple[str, ...]:
    if not isinstance(cli_contract_evidence_items, tuple):
        return ()

    cli_contract_evidence_ids = ()
    for cli_contract_evidence_item in cli_contract_evidence_items:
        cli_contract_evidence_id = _cli_contract_evidence_id_from(
            cli_contract_evidence_item
        )
        if _is_nonblank_string(cli_contract_evidence_id):
            cli_contract_evidence_ids = (
                cli_contract_evidence_ids + (cli_contract_evidence_id,)
            )
    return cli_contract_evidence_ids


def _has_exact_cli_contract_evidence_item_keys(
    cli_contract_evidence_item: dict[str, object],
) -> bool:
    for expected_key in _CLI_CONTRACT_EVIDENCE_ITEM_KEYS:
        if expected_key not in cli_contract_evidence_item:
            return False
    for key in cli_contract_evidence_item:
        if key not in _CLI_CONTRACT_EVIDENCE_ITEM_KEYS:
            return False
    return True


def explain_local_noop_cli_contract_build(
    *,
    run_id: str,
    local_noop_cli_contract_id: str,
    contract_kind: str,
    entry_mode: str,
    mode: str,
    ...
    cli_contract_evidence_items: tuple[dict[str, object], ...],
    required_cli_contract_evidence_ids: tuple[str, ...],
    missing_cli_contract_evidence_ids: tuple[str, ...],
    blocking_cli_contract_evidence_ids: tuple[str, ...],
    created_at: str,
    timestamp_policy: str,
    source_of_truth: tuple[str, ...],
    notes: tuple[str, ...],
) -> dict[str, object]:
    """Explain whether a caller-supplied CLI/manual contract is buildable."""
    ...
    if not _is_nonblank_string(local_noop_cli_contract_id):
        _add_reason(reason_codes, "LOCAL_NOOP_CLI_CONTRACT_ID_MISSING")
        _add_field(
            field_entries,
            reason_code="LOCAL_NOOP_CLI_CONTRACT_ID_MISSING",
            field="local_noop_cli_contract_id",
        )

    if contract_kind != _CONTRACT_KIND:
        _add_reason(reason_codes, "CONTRACT_KIND_NOT_LOCAL_NOOP_CLI_CONTRACT")
        _add_field(
            field_entries,
            reason_code="CONTRACT_KIND_NOT_LOCAL_NOOP_CLI_CONTRACT",
            field="contract_kind",
        )

    if entry_mode not in _SUPPORTED_ENTRY_MODES:
        _add_reason(reason_codes, "ENTRY_MODE_NOT_SUPPORTED")
        _add_field(
            field_entries,
            reason_code="ENTRY_MODE_NOT_SUPPORTED",
            field="entry_mode",
        )
    ...
    if (
        not isinstance(cli_contract_evidence_items, tuple)
        or cli_contract_evidence_items == ()
    ):
        _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_ITEMS_MISSING")
        _add_field(
            field_entries,
            reason_code="CLI_CONTRACT_EVIDENCE_ITEMS_MISSING",
            field="cli_contract_evidence_items",
        )

    if not _is_nonempty_string_tuple(required_cli_contract_evidence_ids):
        _add_reason(reason_codes, "REQUIRED_CLI_CONTRACT_EVIDENCE_IDS_MISSING")
        _add_field(
            field_entries,
            reason_code="REQUIRED_CLI_CONTRACT_EVIDENCE_IDS_MISSING",
            field="required_cli_contract_evidence_ids",
        )

    if (
        not isinstance(missing_cli_contract_evidence_ids, tuple)
        or missing_cli_contract_evidence_ids != ()
    ):
        _add_reason(reason_codes, "MISSING_CLI_CONTRACT_EVIDENCE_IDS_DECLARED")
        _add_field(
            field_entries,
            reason_code="MISSING_CLI_CONTRACT_EVIDENCE_IDS_DECLARED",
            field="missing_cli_contract_evidence_ids",
        )

    known_cli_contract_evidence_ids = _known_cli_contract_evidence_ids(
        cli_contract_evidence_items
    )
    if not isinstance(blocking_cli_contract_evidence_ids, tuple):
        _add_reason(reason_codes, "BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN")
        _add_field(
            field_entries,
            reason_code="BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN",
            field="blocking_cli_contract_evidence_ids",
        )
    else:
        for blocking_cli_contract_evidence_id in blocking_cli_contract_evidence_ids:
            if (
                not _is_nonblank_string(blocking_cli_contract_evidence_id)
                or blocking_cli_contract_evidence_id
                not in known_cli_contract_evidence_ids
            ):
                _add_reason(
                    reason_codes,
                    "BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN",
    ...
    seen_cli_contract_evidence_ids = ()
    if isinstance(cli_contract_evidence_items, tuple):
        for index, cli_contract_evidence_item in enumerate(
            cli_contract_evidence_items
        ):
            cli_contract_evidence_id = _cli_contract_evidence_id_from(
                cli_contract_evidence_item
            ...
            if not isinstance(cli_contract_evidence_item, dict):
                _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT")
                _add_field(
                    field_entries,
                    reason_code="CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT",
                    field=f"cli_contract_evidence_items[{index}]",
                )
                _add_cli_contract_evidence_item_violation(
                    cli_contract_evidence_item_violations,
                    cli_contract_evidence_item_index=index,
                    cli_contract_evidence_id="",
                    reason_code="CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT",
                    field=f"cli_contract_evidence_items[{index}]",
                )
            ...
            if not _has_exact_cli_contract_evidence_item_keys(
                cli_contract_evidence_item
            ):
                _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_ITEM_KEYS_INVALID")
                _add_field(
                    field_entries,
                    reason_code="CLI_CONTRACT_EVIDENCE_ITEM_KEYS_INVALID",
                    field=f"cli_contract_evidence_items[{index}].keys",
                )
                _add_cli_contract_evidence_item_violation(
                    cli_contract_evidence_item_violations,
                    cli_contract_evidence_item_index=index,
            ...
            for field_name, reason_code in _CLI_CONTRACT_EVIDENCE_ITEM_STRING_FIELDS:
                if not _is_nonblank_string(cli_contract_evidence_item.get(field_name)):
                    _add_reason(reason_codes, reason_code)
                    _add_field(
                        field_entries,
                        reason_code=reason_code,
                        field=f"cli_contract_evidence_items[{index}].{field_name}",
                    )
                    _add_cli_contract_evidence_item_violation(
                        cli_contract_evidence_item_violations,
                        cli_contract_evidence_item_index=index,
            ...
            if not _is_nonempty_string_tuple(
                ...
                _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_REFS_MISSING")
                _add_field(
                    field_entries,
                    reason_code="CLI_CONTRACT_EVIDENCE_REFS_MISSING",
                    field=f"cli_contract_evidence_items[{index}].evidence_refs",
                )
                _add_cli_contract_evidence_item_violation(
                    cli_contract_evidence_item_violations,
                    cli_contract_evidence_item_index=index,
            ...
            if (
                _is_nonblank_string(cli_contract_evidence_id)
                and cli_contract_evidence_id in seen_cli_contract_evidence_ids
            ):
                _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_ID_DUPLICATE")
                _add_field(
                    field_entries,
                    reason_code="CLI_CONTRACT_EVIDENCE_ID_DUPLICATE",
                    field=(
                        f"cli_contract_evidence_items[{index}]."
                        "cli_contract_evidence_id"
                    ),
                )
                _add_cli_contract_evidence_item_violation(
                    ...
                    cli_contract_evidence_id=cli_contract_evidence_id,
                    reason_code="CLI_CONTRACT_EVIDENCE_ID_DUPLICATE",
                    field="cli_contract_evidence_id",
                )

            if _is_nonblank_string(cli_contract_evidence_id):
                seen_cli_contract_evidence_ids = (
                    seen_cli_contract_evidence_ids + (cli_contract_evidence_id,)
                )

            if (
                _is_nonempty_string_tuple(required_cli_contract_evidence_ids)
                and _is_nonblank_string(cli_contract_evidence_id)
                and cli_contract_evidence_id not in required_cli_contract_evidence_ids
            ):
                _add_reason(reason_codes, "CLI_CONTRACT_EVIDENCE_ID_NOT_REQUIRED")
                _add_field(
                    field_entries,
                    reason_code="CLI_CONTRACT_EVIDENCE_ID_NOT_REQUIRED",
                    field=(
                        f"cli_contract_evidence_items[{index}]."
                        "cli_contract_evidence_id"
                    ),
            ...
            for field_name in _FORBIDDEN_CLI_CONTRACT_EVIDENCE_ITEM_FIELDS:
                if field_name in cli_contract_evidence_item:
                    _add_reason(
                        reason_codes,
                        "CLI_CONTRACT_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT",
                    )
    ...
    if _is_nonempty_string_tuple(required_cli_contract_evidence_ids):
        for required_cli_contract_evidence_id in required_cli_contract_evidence_ids:
            if required_cli_contract_evidence_id not in known_cli_contract_evidence_ids:
                _add_reason(reason_codes, "REQUIRED_CLI_CONTRACT_EVIDENCE_MISSING")
                _add_field(
                    field_entries,
                    reason_code="REQUIRED_CLI_CONTRACT_EVIDENCE_MISSING",
                    field=(
                        "required_cli_contract_evidence_ids."
                        f"{required_cli_contract_evidence_id}"
                    ),
                )
    ...
    reason_code = (
        "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE"
        if buildable
        else contract_violations[0]
    )

    source = {
        "entry_mode": _safe_string(entry_mode),
    ...
    local_noop_cli_contract = {
        "run_id": _safe_string(run_id),
        "local_noop_cli_contract_id": _safe_string(local_noop_cli_contract_id),
        ...
        "public_url": None,
        "public_url_created": False,
        "cli_contract_evidence_items": _safe_cli_contract_evidence_items(
            cli_contract_evidence_items
        ),
        "required_cli_contract_evidence_ids": _safe_string_tuple(
            required_cli_contract_evidence_ids
        ),
        "missing_cli_contract_evidence_ids": _safe_string_tuple(
            missing_cli_contract_evidence_ids
        ),
        "blocking_cli_contract_evidence_ids": _safe_string_tuple(
            blocking_cli_contract_evidence_ids
        ),
    ...
    return {
        ...
        "source": source,
        "local_noop_cli_contract": local_noop_cli_contract,
        "contract_violations": contract_violations,
        "missing_or_invalid_fields": _ordered_fields(tuple(field_entries)),
        "cli_contract_evidence_item_violations": (
            _ordered_cli_contract_evidence_item_violations(
                tuple(cli_contract_evidence_item_violations)
            )
        ),
...
def is_local_noop_cli_contract_buildable(
    *,
    run_id: str,
    local_noop_cli_contract_id: str,
    contract_kind: str,
    ...
    """Return whether the local noop CLI/manual contract is buildable."""

    return bool(
        explain_local_noop_cli_contract_build(
            run_id=run_id,
            local_noop_cli_contract_id=local_noop_cli_contract_id,
            ...
            cli_contract_evidence_items=cli_contract_evidence_items,
            required_cli_contract_evidence_ids=required_cli_contract_evidence_ids,
            missing_cli_contract_evidence_ids=missing_cli_contract_evidence_ids,
            blocking_cli_contract_evidence_ids=blocking_cli_contract_evidence_ids,
