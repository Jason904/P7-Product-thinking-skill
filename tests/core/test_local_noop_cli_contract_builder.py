"""Tests for the pure local noop CLI/manual contract builder."""
...
from ai_daily_publishing_system.core import (  # noqa: E402
    local_noop_cli_contract_builder as builder,
)
...
LOCAL_NOOP_CLI_CONTRACT_KEYS = (
    "run_id",
    "local_noop_cli_contract_id",
    "contract_kind",
    "entry_mode",
    "mode",
    "expected_terminal_status",
...
CLI_CONTRACT_EVIDENCE_ITEM_KEYS = (
...
CLI_CONTRACT_EVIDENCE_ITEM_VIOLATION_KEYS = (
    "cli_contract_evidence_item_index",
    "cli_contract_evidence_id",
    "reason_code",
    "field",
)
...
FORBIDDEN_CLI_CONTRACT_EVIDENCE_ITEM_FIELDS = (
...
FORBIDDEN_PSEUDO_REASON_CODES = (
    ...
    "RUN_LEDGER_DRAFT_READ_FORBIDDEN",
...
MISSING_CLI_CONTRACT_EVIDENCE_ITEM_KEY_EXPECTATIONS = (
    ...
    ("evidence_refs", "CLI_CONTRACT_EVIDENCE_REFS_MISSING"),
)


def _cli_contract_evidence_item(**overrides):
    values = {
        "cli_contract_evidence_id": "cli-contract-evidence-001",
        "cli_contract_evidence_role": "run_ledger_draft",
        "artifact_ref": "run-ledger-draft-001",
        "artifact_kind": "run_ledger_draft",
        "evidence_status": "passed",
        "producer_ref": "caller-supplied-cli-contract",
        "evidence_refs": ("run-ledger-draft-001#noop-terminal",),
...
def _valid_values():
    return {
        "run_id": "run-001",
        "local_noop_cli_contract_id": "local-noop-cli-contract-001",
        "contract_kind": "local_noop_cli_contract",
        "entry_mode": "cli",
        "mode": "noop",
        ...
        "cli_contract_evidence_items": (_cli_contract_evidence_item(),),
        "required_cli_contract_evidence_ids": ("cli-contract-evidence-001",),
        "missing_cli_contract_evidence_ids": (),
        "blocking_cli_contract_evidence_ids": (),
        "created_at": "caller-supplied-created-at",
        "timestamp_policy": "caller_supplied_no_datetime_parsing",
        "source_of_truth": ("p2d-37",),
...
def _explain(**overrides):
    ...
    values.update(overrides)
    return builder.explain_local_noop_cli_contract_build(**values)
...
def test_reason_code_constants_are_exact_and_stably_prioritized():
    ...
    assert builder.LOCAL_NOOP_CLI_CONTRACT_BUILD_REASON_CODES == (
        REASON_CODES
    )
    assert builder.REASON_PRIORITY == REASON_PRIORITY


def test_valid_cli_contract_is_buildable_with_exact_shapes():
    result = _explain()

    assert result["buildable"] is True
    assert result["reason_code"] == "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE"
    assert result["contract_violations"] == ()
    assert result["missing_or_invalid_fields"] == ()
    assert result["cli_contract_evidence_item_violations"] == ()
    assert tuple(result.keys()) == RESULT_KEYS
    assert tuple(result["source"].keys()) == SOURCE_KEYS
    assert tuple(result["local_noop_cli_contract"].keys()) == (
        LOCAL_NOOP_CLI_CONTRACT_KEYS
    )
    assert tuple(
        result["local_noop_cli_contract"]["cli_contract_evidence_items"][0].keys()
    ) == CLI_CONTRACT_EVIDENCE_ITEM_KEYS
    assert result["source"]["public_url"] is None
    assert result["local_noop_cli_contract"]["public_url"] is None
    assert "public_url_is_null" not in result
    assert "public_url_is_null" not in result["source"]
    assert "public_url_is_null" not in result["local_noop_cli_contract"]


def test_valid_manual_mode_is_buildable_and_declarative_only():
    result = _explain(entry_mode="manual")

    assert result["buildable"] is True
    assert result["source"]["entry_mode"] == "manual"
    assert result["local_noop_cli_contract"]["entry_mode"] == "manual"
    assert "entry_mode_is_declarative_only" in result["invariant_refs"]
    assert "buildable_not_manual_executed" in result["invariant_refs"]
...
def test_public_api_is_keyword_only_and_bool_wrapper_matches_explain():
    ...
    assert (
        builder.explain_local_noop_cli_contract_build.__code__.co_argcount
        == 0
    )
    assert (
        builder.explain_local_noop_cli_contract_build.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    assert (
        builder.is_local_noop_cli_contract_buildable.__code__.co_argcount
        == 0
    )
    assert (
        builder.is_local_noop_cli_contract_buildable.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    ...
    for values in cases:
        explanation = builder.explain_local_noop_cli_contract_build(**values)
        assert (
            builder.is_local_noop_cli_contract_buildable(**values)
...
def test_required_lockin_markers_block_when_invalid():
    cases = (
        (
            {"contract_kind": "local_noop_manual_entry_contract"},
            "CONTRACT_KIND_NOT_LOCAL_NOOP_CLI_CONTRACT",
            "contract_kind",
        ),
        ({"entry_mode": "api"}, "ENTRY_MODE_NOT_SUPPORTED", "entry_mode"),
        ({"entry_mode": ""}, "ENTRY_MODE_NOT_SUPPORTED", "entry_mode"),
        ({"entry_mode": 1}, "ENTRY_MODE_NOT_SUPPORTED", "entry_mode"),
...
def test_pass_published_status_is_blocked_and_suppressed_from_payload():
    ...
    assert result["local_noop_cli_contract"]["expected_terminal_status"] == ""
...
def test_public_url_created_is_blocked_and_suppressed_from_payload():
    ...
    assert result["buildable"] is False
    assert "PUBLIC_URL_CREATED_NOT_FALSE" in result["contract_violations"]
    assert result["source"]["public_url_created"] is False
    assert result["local_noop_cli_contract"]["public_url_created"] is False
...
def test_each_upstream_ref_is_required_and_opaque():
    ...
    assert opaque["source"]["run_ledger_draft_ref"] == "opaque-ledger-draft"


def test_refs_status_and_evidence_refs_are_opaque():
    result = _explain(
        cli_contract_evidence_items=(
            _cli_contract_evidence_item(
    ...
    assert result["buildable"] is True
    assert (
        result["local_noop_cli_contract"]["cli_contract_evidence_items"][0][
            "evidence_status"
        ]
        == "failed"
    )
...
def test_known_blocking_cli_contract_evidence_ids_still_buildable():
    result = _explain(
        blocking_cli_contract_evidence_ids=("cli-contract-evidence-001",),
        cli_contract_evidence_items=(
            _cli_contract_evidence_item(evidence_status="failed"),
        ),
    )

    assert result["buildable"] is True
    assert result["local_noop_cli_contract"][
        "blocking_cli_contract_evidence_ids"
    ] == ("cli-contract-evidence-001",)


def test_unknown_blank_or_non_tuple_blocking_ids_block_buildability():
    cases = (
        ("missing-cli-contract-evidence",),
        ("",),
        (1,),
        ["cli-contract-evidence-001"],
    )

    for blocking_cli_contract_evidence_ids in cases:
        result = _explain(
            blocking_cli_contract_evidence_ids=blocking_cli_contract_evidence_ids
        )

        assert result["buildable"] is False
        assert "BLOCKING_CLI_CONTRACT_EVIDENCE_ID_UNKNOWN" in (
            result["contract_violations"]
        )
        assert "blocking_cli_contract_evidence_ids" in (
...
def test_required_top_level_field_violations_are_collected():
    cases = (
        ...
        (
            {"missing_cli_contract_evidence_ids": ("cli-contract-evidence-002",)},
            "MISSING_CLI_CONTRACT_EVIDENCE_IDS_DECLARED",
            "missing_cli_contract_evidence_ids",
        ),
        (
...
def test_cli_contract_evidence_id_uniqueness_required_boundary_and_presence():
    duplicate = _explain(
        cli_contract_evidence_items=(
            _cli_contract_evidence_item(),
            _cli_contract_evidence_item(),
    ...
    not_required = _explain(
        cli_contract_evidence_items=(
            _cli_contract_evidence_item(
                cli_contract_evidence_id="cli-contract-evidence-002"
            ),
        ),
        required_cli_contract_evidence_ids=("cli-contract-evidence-001",),
    )
    required_missing = _explain(
        required_cli_contract_evidence_ids=(
            "cli-contract-evidence-001",
            "cli-contract-evidence-002",
        )
    )

    assert "CLI_CONTRACT_EVIDENCE_ID_DUPLICATE" in duplicate["contract_violations"]
    assert "cli_contract_evidence_items[1].cli_contract_evidence_id" in (
        duplicate["missing_or_invalid_fields"]
    )
    assert "CLI_CONTRACT_EVIDENCE_ID_NOT_REQUIRED" in (
        not_required["contract_violations"]
    )
    assert "cli_contract_evidence_items[0].cli_contract_evidence_id" in (
        not_required["missing_or_invalid_fields"]
    )
    assert "REQUIRED_CLI_CONTRACT_EVIDENCE_MISSING" in (
        required_missing["contract_violations"]
    )
    assert "required_cli_contract_evidence_ids.cli-contract-evidence-002" in (
        required_missing["missing_or_invalid_fields"]
    )


def test_cli_contract_evidence_item_fields_and_missing_keys_reported():
    string_field_cases = (
        ("cli_contract_evidence_id", "", "CLI_CONTRACT_EVIDENCE_ID_MISSING"),
        ("cli_contract_evidence_role", "", "CLI_CONTRACT_EVIDENCE_ROLE_MISSING"),
        ("artifact_ref", "", "CLI_CONTRACT_EVIDENCE_ARTIFACT_REF_MISSING"),
        ...
        ("evidence_refs", (), "CLI_CONTRACT_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ("",), "CLI_CONTRACT_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ["ref"], "CLI_CONTRACT_EVIDENCE_REFS_MISSING"),
    )

    for field, value, reason_code in string_field_cases:
        result = _explain(
            cli_contract_evidence_items=(
                _cli_contract_evidence_item(**{field: value}),
            )
        )

        assert result["buildable"] is False
        assert reason_code in result["contract_violations"]
        assert f"cli_contract_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
        )
        assert tuple(
            result["cli_contract_evidence_item_violations"][0].keys()
        ) == CLI_CONTRACT_EVIDENCE_ITEM_VIOLATION_KEYS

    for missing_key, reason_code in MISSING_CLI_CONTRACT_EVIDENCE_ITEM_KEY_EXPECTATIONS:
        cli_contract_evidence_item = _cli_contract_evidence_item()
        del cli_contract_evidence_item[missing_key]
        result = _explain(cli_contract_evidence_items=(cli_contract_evidence_item,))

        assert result["buildable"] is False
        assert "CLI_CONTRACT_EVIDENCE_ITEM_KEYS_INVALID" in (
            result["contract_violations"]
        )
        assert reason_code in result["contract_violations"]
        assert f"cli_contract_evidence_items[0].{missing_key}" in (
            result["missing_or_invalid_fields"]
        )
        assert "cli_contract_evidence_items[0].keys" in (
            result["missing_or_invalid_fields"]
        )


def test_non_dict_cli_contract_evidence_item_records_violation_shape():
    result = _explain(cli_contract_evidence_items=("not-an-evidence-item",))

    assert result["buildable"] is False
    assert "CLI_CONTRACT_EVIDENCE_ITEM_NOT_DICT" in result["contract_violations"]
    assert "cli_contract_evidence_items[0]" in result["missing_or_invalid_fields"]
    assert tuple(result["cli_contract_evidence_item_violations"][0].keys()) == (
        CLI_CONTRACT_EVIDENCE_ITEM_VIOLATION_KEYS
    )
    assert result["local_noop_cli_contract"]["cli_contract_evidence_items"][0] == {
        "cli_contract_evidence_id": "",
        "cli_contract_evidence_role": "",
        "artifact_ref": "",
        "artifact_kind": "",
...
def test_forbidden_raw_execution_cli_url_and_publication_fields_are_suppressed():
    for field in FORBIDDEN_CLI_CONTRACT_EVIDENCE_ITEM_FIELDS:
        result = _explain(
            cli_contract_evidence_items=(
                _cli_contract_evidence_item(**{field: "forbidden"}),
            )
        )
        ...
        assert "CLI_CONTRACT_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT" in (
            result["contract_violations"]
        )
        assert f"cli_contract_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
...
def test_output_does_not_expose_public_url_is_null_or_extra_url_values():
    ...
    forbidden_url = _explain(
        cli_contract_evidence_items=(
            _cli_contract_evidence_item(public_url_value="https://example.test"),
        )
    )
    ...
    assert forbidden_url["local_noop_cli_contract"]["public_url"] is None


def test_reason_catalog_priority_and_forbidden_pseudo_reason_codes():
    assert builder.REASON_CODES == REASON_CODES
    assert builder.LOCAL_NOOP_CLI_CONTRACT_BUILD_REASON_CODES == REASON_CODES
    assert builder.REASON_PRIORITY == REASON_PRIORITY
...
def test_all_violations_are_priority_ordered_and_details_present():
    first_item = _cli_contract_evidence_item(
        cli_contract_evidence_id="",
        cli_contract_evidence_role="",
        artifact_ref="",
        artifact_kind="",
    ...
    second_item = _cli_contract_evidence_item(
        cli_contract_evidence_id="extra-evidence"
    )
    result = _explain(
        run_id="",
        local_noop_cli_contract_id="",
        ...
        public_url_is_null=False,
        public_url_created=True,
        cli_contract_evidence_items=(first_item, second_item),
        required_cli_contract_evidence_ids=(
            "cli-contract-evidence-001",
            "cli-contract-evidence-002",
        ),
        missing_cli_contract_evidence_ids=("cli-contract-evidence-002",),
        blocking_cli_contract_evidence_ids=("missing-block",),
        created_at="",
        timestamp_policy="",
        source_of_truth=(),
    )
    ...
    assert "cli_contract_evidence_items[0].raw_artifact_content" in (
        result["missing_or_invalid_fields"]
    )
    assert "cli_contract_evidence_items[1].cli_contract_evidence_id" in (
        result["missing_or_invalid_fields"]
    ...
    assert result["cli_contract_evidence_item_violations"] != ()
    for violation in result["cli_contract_evidence_item_violations"]:
...
def test_payload_key_traversal_skips_invariants_instead_of_blanket_scans():
    ...
    assert "local_noop_cli_contract_not_command_execution" in (
