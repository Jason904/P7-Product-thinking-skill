"""Tests for the pure local noop runner skeleton builder."""
...
from ai_daily_publishing_system.core import (  # noqa: E402
    local_noop_runner_skeleton_builder as builder,
)
...
LOCAL_NOOP_RUNNER_SKELETON_KEYS = (
    "run_id",
...
SKELETON_EVIDENCE_ITEM_VIOLATION_KEYS = (
    "skeleton_evidence_item_index",
    "skeleton_evidence_id",
    "reason_code",
    "field",
)
...
FORBIDDEN_SKELETON_EVIDENCE_ITEM_FIELDS = (
...
FORBIDDEN_PSEUDO_REASON_CODES = (
    ...
    "LOCAL_NOOP_CLI_CONTRACT_READ_FORBIDDEN",
...
MISSING_SKELETON_EVIDENCE_ITEM_KEY_EXPECTATIONS = (
    ...
    ("evidence_refs", "SKELETON_EVIDENCE_REFS_MISSING"),
)


def _skeleton_evidence_item(**overrides):
    values = {
        "skeleton_evidence_id": "skeleton-evidence-001",
        "skeleton_evidence_role": "local_noop_cli_contract",
        "artifact_ref": "local-noop-cli-contract-001",
        "artifact_kind": "local_noop_cli_contract",
        "evidence_status": "passed",
        "producer_ref": "caller-supplied-runner-skeleton",
        "evidence_refs": ("local-noop-cli-contract-001#noop-terminal",),
...
def _valid_values():
    return {
        "run_id": "run-001",
        "local_noop_runner_skeleton_id": "local-noop-runner-skeleton-001",
        "skeleton_kind": "local_noop_runner_skeleton",
        "mode": "noop",
        ...
        "skeleton_evidence_items": (_skeleton_evidence_item(),),
        "required_skeleton_evidence_ids": ("skeleton-evidence-001",),
        "missing_skeleton_evidence_ids": (),
        "blocking_skeleton_evidence_ids": (),
        "created_at": "caller-supplied-created-at",
        "timestamp_policy": "caller_supplied_no_datetime_parsing",
        "source_of_truth": ("p2d-38",),
...
def _explain(**overrides):
    ...
    values.update(overrides)
    return builder.explain_local_noop_runner_skeleton_build(**values)
...
def test_reason_code_constants_are_exact_and_stably_prioritized():
    ...
    assert builder.LOCAL_NOOP_RUNNER_SKELETON_BUILD_REASON_CODES == (
        REASON_CODES
    )
    assert builder.REASON_PRIORITY == REASON_PRIORITY


def test_valid_runner_skeleton_is_buildable_with_exact_shapes():
    result = _explain()

    assert result["buildable"] is True
    assert result["reason_code"] == "LOCAL_NOOP_RUNNER_SKELETON_BUILDABLE"
    assert result["skeleton_violations"] == ()
    assert result["missing_or_invalid_fields"] == ()
    assert result["skeleton_evidence_item_violations"] == ()
    assert tuple(result.keys()) == RESULT_KEYS
    assert tuple(result["source"].keys()) == SOURCE_KEYS
    assert tuple(result["local_noop_runner_skeleton"].keys()) == (
        LOCAL_NOOP_RUNNER_SKELETON_KEYS
    )
    assert tuple(
        result["local_noop_runner_skeleton"]["skeleton_evidence_items"][0].keys()
    ) == SKELETON_EVIDENCE_ITEM_KEYS
    assert result["source"]["public_url"] is None
    assert result["local_noop_runner_skeleton"]["public_url"] is None
    assert result["source"]["public_url_created"] is False
    assert result["local_noop_runner_skeleton"]["public_url_created"] is False
    assert "public_url_is_null" not in result
    assert "public_url_is_null" not in result["source"]
    assert "public_url_is_null" not in result["local_noop_runner_skeleton"]
...
def test_public_api_is_keyword_only_and_bool_wrapper_matches_explain():
    ...
    assert (
        builder.explain_local_noop_runner_skeleton_build.__code__.co_argcount
        == 0
    )
    assert (
        builder.explain_local_noop_runner_skeleton_build.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    assert (
        builder.is_local_noop_runner_skeleton_buildable.__code__.co_argcount
        == 0
    )
    assert (
        builder.is_local_noop_runner_skeleton_buildable.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    ...
    for values in cases:
        explanation = builder.explain_local_noop_runner_skeleton_build(**values)
        assert (
            builder.is_local_noop_runner_skeleton_buildable(**values)
...
def test_required_lockin_markers_block_when_invalid():
    cases = (
        (
            {"skeleton_kind": "local_noop_runner"},
            "SKELETON_KIND_NOT_LOCAL_NOOP_RUNNER_SKELETON",
            "skeleton_kind",
        ),
        ...
        (
            {"local_noop_cli_contract_buildable_marker": 1},
            "LOCAL_NOOP_CLI_CONTRACT_BUILDABLE_MARKER_NOT_TRUE",
            "local_noop_cli_contract_buildable_marker",
        ),
        (
    ...
    for overrides, reason_code, field in cases:
        ...
        assert reason_code in result["skeleton_violations"]
...
def test_pass_published_status_is_blocked_and_suppressed_from_payload():
    ...
    assert "EXPECTED_TERMINAL_STATUS_NOT_NOOP_COMPLETED" in (
        result["skeleton_violations"]
    )
    assert result["source"]["expected_terminal_status"] == ""
    assert result["local_noop_runner_skeleton"]["expected_terminal_status"] == ""
...
def test_public_url_created_is_blocked_and_suppressed_from_payload():
    ...
    assert result["buildable"] is False
    assert "PUBLIC_URL_CREATED_NOT_FALSE" in result["skeleton_violations"]
    assert result["source"]["public_url_created"] is False
    assert result["local_noop_runner_skeleton"]["public_url_created"] is False
...
def test_each_upstream_ref_is_required_and_opaque():
    ...
    assert opaque["source"]["local_noop_cli_contract_ref"] == (
        "opaque-cli-contract"
    )
...
def test_refs_status_and_evidence_refs_are_opaque():
    ...
    assert result["buildable"] is True
    assert (
        result["local_noop_runner_skeleton"]["skeleton_evidence_items"][0][
            "evidence_status"
        ]
        == "failed"
    )
...
def test_known_blocking_skeleton_evidence_ids_still_buildable():
    result = _explain(
        blocking_skeleton_evidence_ids=("skeleton-evidence-001",),
        skeleton_evidence_items=(
            _skeleton_evidence_item(evidence_status="failed"),
        ),
    )

    assert result["buildable"] is True
    assert result["local_noop_runner_skeleton"][
        "blocking_skeleton_evidence_ids"
    ] == ("skeleton-evidence-001",)


def test_unknown_blank_or_non_tuple_blocking_ids_block_buildability():
    cases = (
        ("missing-skeleton-evidence",),
        ("",),
        (1,),
        ["skeleton-evidence-001"],
    )

    for blocking_skeleton_evidence_ids in cases:
        result = _explain(
            blocking_skeleton_evidence_ids=blocking_skeleton_evidence_ids
        )

        assert result["buildable"] is False
        assert "BLOCKING_SKELETON_EVIDENCE_ID_UNKNOWN" in (
            result["skeleton_violations"]
        )
        assert "blocking_skeleton_evidence_ids" in (
...
def test_required_top_level_field_violations_are_collected():
    cases = (
        ...
        (
            {"missing_skeleton_evidence_ids": ("skeleton-evidence-002",)},
            "MISSING_SKELETON_EVIDENCE_IDS_DECLARED",
            "missing_skeleton_evidence_ids",
        ),
        (
...
def test_skeleton_evidence_id_uniqueness_required_boundary_and_presence():
    duplicate = _explain(
        skeleton_evidence_items=(
            _skeleton_evidence_item(),
            _skeleton_evidence_item(),
    ...
    not_required = _explain(
        skeleton_evidence_items=(
            _skeleton_evidence_item(
                skeleton_evidence_id="skeleton-evidence-002"
            ),
        ),
        required_skeleton_evidence_ids=("skeleton-evidence-001",),
    )
    required_missing = _explain(
        required_skeleton_evidence_ids=(
            "skeleton-evidence-001",
            "skeleton-evidence-002",
        )
    )

    assert "SKELETON_EVIDENCE_ID_DUPLICATE" in duplicate["skeleton_violations"]
    assert "skeleton_evidence_items[1].skeleton_evidence_id" in (
        duplicate["missing_or_invalid_fields"]
    )
    assert "SKELETON_EVIDENCE_ID_NOT_REQUIRED" in (
        not_required["skeleton_violations"]
    )
    assert "skeleton_evidence_items[0].skeleton_evidence_id" in (
        not_required["missing_or_invalid_fields"]
    )
    assert "REQUIRED_SKELETON_EVIDENCE_MISSING" in (
        required_missing["skeleton_violations"]
    )
    assert "required_skeleton_evidence_ids.skeleton-evidence-002" in (
        required_missing["missing_or_invalid_fields"]
    )


def test_skeleton_evidence_item_fields_and_missing_keys_reported():
    string_field_cases = (
        ("skeleton_evidence_id", "", "SKELETON_EVIDENCE_ID_MISSING"),
        ("skeleton_evidence_role", "", "SKELETON_EVIDENCE_ROLE_MISSING"),
        ("artifact_ref", "", "SKELETON_EVIDENCE_ARTIFACT_REF_MISSING"),
        ...
        ("evidence_refs", (), "SKELETON_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ("",), "SKELETON_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ["ref"], "SKELETON_EVIDENCE_REFS_MISSING"),
    )

    for field, value, reason_code in string_field_cases:
        result = _explain(
            skeleton_evidence_items=(
                _skeleton_evidence_item(**{field: value}),
            )
        )

        assert result["buildable"] is False
        assert reason_code in result["skeleton_violations"]
        assert f"skeleton_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
        )
        assert tuple(
            result["skeleton_evidence_item_violations"][0].keys()
        ) == SKELETON_EVIDENCE_ITEM_VIOLATION_KEYS

    for missing_key, reason_code in MISSING_SKELETON_EVIDENCE_ITEM_KEY_EXPECTATIONS:
        skeleton_evidence_item = _skeleton_evidence_item()
        del skeleton_evidence_item[missing_key]
        result = _explain(skeleton_evidence_items=(skeleton_evidence_item,))

        assert result["buildable"] is False
        assert "SKELETON_EVIDENCE_ITEM_KEYS_INVALID" in (
            result["skeleton_violations"]
        )
        assert reason_code in result["skeleton_violations"]
        assert f"skeleton_evidence_items[0].{missing_key}" in (
            result["missing_or_invalid_fields"]
        )
        assert "skeleton_evidence_items[0].keys" in (
            result["missing_or_invalid_fields"]
        )


def test_non_dict_skeleton_evidence_item_records_violation_shape():
    result = _explain(skeleton_evidence_items=("not-an-evidence-item",))

    assert result["buildable"] is False
    assert "SKELETON_EVIDENCE_ITEM_NOT_DICT" in result["skeleton_violations"]
    assert "skeleton_evidence_items[0]" in result["missing_or_invalid_fields"]
    assert tuple(result["skeleton_evidence_item_violations"][0].keys()) == (
        SKELETON_EVIDENCE_ITEM_VIOLATION_KEYS
    )
    assert result["local_noop_runner_skeleton"]["skeleton_evidence_items"][0] == {
        "skeleton_evidence_id": "",
        "skeleton_evidence_role": "",
        "artifact_ref": "",
        "artifact_kind": "",
...
def test_forbidden_raw_execution_cli_url_and_publication_fields_are_suppressed():
    for field in FORBIDDEN_SKELETON_EVIDENCE_ITEM_FIELDS:
        result = _explain(
            skeleton_evidence_items=(
                _skeleton_evidence_item(**{field: "forbidden"}),
            )
        )
        ...
        assert "SKELETON_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT" in (
            result["skeleton_violations"]
        )
        assert f"skeleton_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
...
def test_output_does_not_expose_public_url_is_null_or_extra_url_values():
    ...
    forbidden_url = _explain(
        skeleton_evidence_items=(
            _skeleton_evidence_item(public_url_value="https://example.test"),
        )
    )
    ...
    assert forbidden_url["local_noop_runner_skeleton"]["public_url"] is None


def test_reason_catalog_priority_and_forbidden_pseudo_reason_codes():
    assert builder.REASON_CODES == REASON_CODES
    assert builder.LOCAL_NOOP_RUNNER_SKELETON_BUILD_REASON_CODES == REASON_CODES
    assert builder.REASON_PRIORITY == REASON_PRIORITY
...
def test_all_violations_are_priority_ordered_and_details_present():
    first_item = _skeleton_evidence_item(
        skeleton_evidence_id="",
        skeleton_evidence_role="",
        artifact_ref="",
        artifact_kind="",
    ...
    second_item = _skeleton_evidence_item(
        skeleton_evidence_id="extra-evidence"
    )
    result = _explain(
        run_id="",
        local_noop_runner_skeleton_id="",
        ...
        public_url_is_null=False,
        public_url_created=True,
        skeleton_evidence_items=(first_item, second_item),
        required_skeleton_evidence_ids=(
            "skeleton-evidence-001",
            "skeleton-evidence-002",
        ),
        missing_skeleton_evidence_ids=("skeleton-evidence-002",),
        blocking_skeleton_evidence_ids=("missing-block",),
        created_at="",
        timestamp_policy="",
        source_of_truth=(),
    )
    expected_order = tuple(
        ...
        if reason_code != "LOCAL_NOOP_RUNNER_SKELETON_BUILDABLE"
        and reason_code in result["skeleton_violations"]
    )

    assert result["skeleton_violations"] == expected_order
    assert result["reason_code"] == result["skeleton_violations"][0]
    assert "skeleton_evidence_items[0].raw_artifact_content" in (
        result["missing_or_invalid_fields"]
    )
    assert "skeleton_evidence_items[1].skeleton_evidence_id" in (
        result["missing_or_invalid_fields"]
    ...
    assert result["skeleton_evidence_item_violations"] != ()
    for violation in result["skeleton_evidence_item_violations"]:
...
def test_payload_key_traversal_skips_invariants_instead_of_blanket_scans():
    ...
    assert "local_noop_runner_skeleton_not_command_execution" in (
