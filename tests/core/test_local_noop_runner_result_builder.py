"""Tests for the pure local noop runner result buildability builder."""
...
from ai_daily_publishing_system.core import (  # noqa: E402
    local_noop_runner_result_builder as builder,
)
...
RUNNER_EVIDENCE_ITEM_VIOLATION_KEYS = (
    "runner_evidence_item_index",
    "runner_evidence_id",
    "reason_code",
    "field",
)
...
FORBIDDEN_RUNNER_EVIDENCE_ITEM_FIELDS = (
...
MISSING_RUNNER_EVIDENCE_ITEM_KEY_EXPECTATIONS = (
    ...
    ("evidence_refs", "RUNNER_EVIDENCE_REFS_MISSING"),
)


def _runner_evidence_item(**overrides):
    values = {
        "runner_evidence_id": "runner-evidence-001",
        "runner_evidence_role": "local_noop_runner_result",
        "artifact_ref": "local-noop-e2e-contract-001",
        "artifact_kind": "local_noop_e2e_contract",
        "evidence_status": "passed",
        "producer_ref": "caller-supplied-local-noop-runner-result",
        "evidence_refs": ("local-noop-e2e-contract-001#noop-contract",),
...
def _valid_values():
    return {
        "run_id": "run-001",
        "local_noop_runner_result_id": "local-noop-runner-result-001",
        ...
        "runner_evidence_items": (_runner_evidence_item(),),
...
def _explain(**overrides):
    ...
    values.update(overrides)
    return builder.explain_local_noop_runner_result_build(**values)
...
def test_reason_code_constants_are_exact_and_stably_prioritized():
    ...
    assert builder.LOCAL_NOOP_RUNNER_RESULT_BUILD_REASON_CODES == (
        REASON_CODES
    )
    assert builder.REASON_PRIORITY == REASON_PRIORITY


def test_valid_local_noop_runner_result_is_buildable_with_exact_shapes():
    result = _explain()

    assert result["buildable"] is True
    assert result["reason_code"] == "LOCAL_NOOP_RUNNER_RESULT_BUILDABLE"
    assert result["result_violations"] == ()
    assert result["missing_or_invalid_fields"] == ()
    assert result["runner_evidence_item_violations"] == ()
    assert tuple(result.keys()) == RESULT_KEYS
    assert tuple(result["source"].keys()) == SOURCE_KEYS
    assert tuple(result["local_noop_runner_result"].keys()) == (
        LOCAL_NOOP_RUNNER_RESULT_KEYS
    )
    assert tuple(
        result["local_noop_runner_result"]["runner_evidence_items"][0].keys()
    ) == RUNNER_EVIDENCE_ITEM_KEYS
    ...
    assert "public_url_is_null" not in result["local_noop_runner_result"]


def test_public_api_is_keyword_only_and_bool_wrapper_matches_explain():
    expected_kwonly = len(LOCAL_NOOP_RUNNER_RESULT_KEYS)

    assert (
        builder.explain_local_noop_runner_result_build.__code__.co_argcount
        == 0
    )
    assert (
        builder.explain_local_noop_runner_result_build.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    assert (
        builder.is_local_noop_runner_result_buildable.__code__.co_argcount
        == 0
    )
    assert (
        builder.is_local_noop_runner_result_buildable.__code__
        .co_kwonlyargcount
        == expected_kwonly
    )
    ...
    for values in cases:
        explanation = builder.explain_local_noop_runner_result_build(**values)
        assert (
            builder.is_local_noop_runner_result_buildable(**values)
...
def test_required_lockin_markers_block_when_invalid():
    cases = (
        ...
        (
            {"local_noop_e2e_contract_buildable_marker": 1},
            "LOCAL_NOOP_E2E_CONTRACT_BUILDABLE_MARKER_NOT_TRUE",
            "local_noop_e2e_contract_buildable_marker",
        ),
        (
...
def test_refs_status_and_evidence_refs_are_opaque():
    result = _explain(
        local_noop_e2e_contract_ref="opaque-e2e-contract-ref",
        runner_evidence_items=(
            _runner_evidence_item(
    ...
    assert result["buildable"] is True
    assert (
        result["source"]["local_noop_e2e_contract_ref"]
        == "opaque-e2e-contract-ref"
    )
    assert (
        result["local_noop_runner_result"]["runner_evidence_items"][0][
            "evidence_status"
        ]
        == "failed"
    )
...
def test_known_blocking_runner_evidence_ids_still_buildable():
    result = _explain(
        blocking_runner_evidence_ids=("runner-evidence-001",),
        runner_evidence_items=(
            _runner_evidence_item(evidence_status="failed"),
        ),
    )
...
def test_unknown_blank_or_non_tuple_blocking_ids_block_buildability():
    ...
    for blocking_runner_evidence_ids in cases:
        result = _explain(
            blocking_runner_evidence_ids=blocking_runner_evidence_ids
        )

        assert result["buildable"] is False
        assert "BLOCKING_RUNNER_EVIDENCE_ID_UNKNOWN" in (
            result["result_violations"]
        )
        assert "blocking_runner_evidence_ids" in (
...
def test_required_top_level_field_violations_are_collected():
    cases = (
        ...
        (
            {"missing_runner_evidence_ids": ("runner-evidence-002",)},
            "MISSING_RUNNER_EVIDENCE_IDS_DECLARED",
            "missing_runner_evidence_ids",
        ),
        (
...
def test_runner_evidence_id_uniqueness_required_boundary_and_presence():
    duplicate = _explain(
        runner_evidence_items=(
            _runner_evidence_item(),
            _runner_evidence_item(),
    ...
    not_required = _explain(
        runner_evidence_items=(
            _runner_evidence_item(
                runner_evidence_id="runner-evidence-002"
            ),
        ),
        required_runner_evidence_ids=("runner-evidence-001",),
    )
    ...
    assert "RUNNER_EVIDENCE_ID_DUPLICATE" in duplicate["result_violations"]
    assert "runner_evidence_items[1].runner_evidence_id" in (
        duplicate["missing_or_invalid_fields"]
    )
    assert "RUNNER_EVIDENCE_ID_NOT_REQUIRED" in (
        not_required["result_violations"]
    )
    assert "runner_evidence_items[0].runner_evidence_id" in (
        not_required["missing_or_invalid_fields"]
    )
    assert "REQUIRED_RUNNER_EVIDENCE_MISSING" in (
        required_missing["result_violations"]
    )
    assert "required_runner_evidence_ids.runner-evidence-002" in (
        required_missing["missing_or_invalid_fields"]
    )


def test_runner_evidence_item_fields_and_missing_keys_reported():
    string_field_cases = (
        ("runner_evidence_id", "", "RUNNER_EVIDENCE_ID_MISSING"),
        ("runner_evidence_role", "", "RUNNER_EVIDENCE_ROLE_MISSING"),
        ("artifact_ref", "", "RUNNER_EVIDENCE_ARTIFACT_REF_MISSING"),
        ...
        ("evidence_refs", (), "RUNNER_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ("",), "RUNNER_EVIDENCE_REFS_MISSING"),
        ("evidence_refs", ["ref"], "RUNNER_EVIDENCE_REFS_MISSING"),
    )

    for field, value, reason_code in string_field_cases:
        result = _explain(
            runner_evidence_items=(
                _runner_evidence_item(**{field: value}),
            )
        )

        assert result["buildable"] is False
        assert reason_code in result["result_violations"]
        assert f"runner_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
        )
        assert tuple(
            result["runner_evidence_item_violations"][0].keys()
        ) == RUNNER_EVIDENCE_ITEM_VIOLATION_KEYS

    for missing_key, reason_code in MISSING_RUNNER_EVIDENCE_ITEM_KEY_EXPECTATIONS:
        runner_evidence_item = _runner_evidence_item()
        del runner_evidence_item[missing_key]
        result = _explain(runner_evidence_items=(runner_evidence_item,))

        assert result["buildable"] is False
        assert "RUNNER_EVIDENCE_ITEM_KEYS_INVALID" in (
            result["result_violations"]
        )
        assert reason_code in result["result_violations"]
        assert f"runner_evidence_items[0].{missing_key}" in (
            result["missing_or_invalid_fields"]
        )
        assert "runner_evidence_items[0].keys" in (
            result["missing_or_invalid_fields"]
        )


def test_non_dict_runner_evidence_item_records_violation_shape():
    result = _explain(runner_evidence_items=("not-an-evidence-item",))

    assert result["buildable"] is False
    assert "RUNNER_EVIDENCE_ITEM_NOT_DICT" in result["result_violations"]
    assert "runner_evidence_items[0]" in result["missing_or_invalid_fields"]
    assert tuple(result["runner_evidence_item_violations"][0].keys()) == (
        RUNNER_EVIDENCE_ITEM_VIOLATION_KEYS
    )
...
def test_forbidden_raw_execution_url_and_publication_fields_are_suppressed():
    for field in FORBIDDEN_RUNNER_EVIDENCE_ITEM_FIELDS:
        result = _explain(
            runner_evidence_items=(
                _runner_evidence_item(**{field: "forbidden"}),
            )
        )
        ...
        assert "RUNNER_EVIDENCE_ITEM_FORBIDDEN_RAW_FIELD_PRESENT" in (
            result["result_violations"]
        )
        assert f"runner_evidence_items[0].{field}" in (
            result["missing_or_invalid_fields"]
...
def test_output_does_not_expose_public_url_is_null_or_extra_url_values():
    ...
    forbidden_url = _explain(
        runner_evidence_items=(
            _runner_evidence_item(public_url_value="https://example.test"),
        )
    )
    ...
    assert forbidden_url["local_noop_runner_result"]["public_url"] is None


def test_reason_catalog_priority_and_forbidden_pseudo_reason_codes():
    assert builder.REASON_CODES == REASON_CODES
    assert builder.LOCAL_NOOP_RUNNER_RESULT_BUILD_REASON_CODES == REASON_CODES
    assert builder.REASON_PRIORITY == REASON_PRIORITY
...
def test_all_violations_are_priority_ordered_and_details_present():
    first_item = _runner_evidence_item(
        runner_evidence_id="",
        runner_evidence_role="",
        artifact_ref="",
        artifact_kind="",
    ...
    second_item = _runner_evidence_item(
        runner_evidence_id="extra-evidence"
    )
    result = _explain(
        run_id="",
        local_noop_runner_result_id="",
        ...
        public_url_is_null=False,
        public_url_created=True,
        runner_evidence_items=(first_item, second_item),
        required_runner_evidence_ids=(
            "runner-evidence-001",
            "runner-evidence-002",
        ),
        missing_runner_evidence_ids=("runner-evidence-002",),
        blocking_runner_evidence_ids=("missing-block",),
        created_at="",
        timestamp_policy="",
        source_of_truth=(),
    )
    expected_order = tuple(
        ...
        and reason_code in result["result_violations"]
    )

    assert result["result_violations"] == expected_order
    assert result["reason_code"] == result["result_violations"][0]
    assert "runner_evidence_items[0].raw_artifact_content" in (
        result["missing_or_invalid_fields"]
    )
    assert "runner_evidence_items[1].runner_evidence_id" in (
        result["missing_or_invalid_fields"]
    ...
    assert result["runner_evidence_item_violations"] != ()
    for violation in result["runner_evidence_item_violations"]:
...
def test_payload_key_traversal_skips_invariants_instead_of_blanket_scans():
    ...
    assert "local_noop_runner_result_not_runner_execution" in (
